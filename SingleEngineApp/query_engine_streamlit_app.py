...
Benutzerfreundliche Web-Oberfläche für den Query Agent
"""
...
def main():
    ...
    st.markdown("KI-Agent mit leistungsstarker Web-Suchfähigkeit")
    st.markdown("Breite Erfassung offizieller Berichte und Nachrichten – nationale und internationale Quellen kombiniert")
    ...
    if start_research:
        ...
        # API-Schlüssel für DeepSeek prüfen
        if not settings.QUERY_ENGINE_API_KEY:
            st.error("Bitte QUERY_ENGINE_API_KEY in den Umgebungsvariablen setzen")
            return
        if not settings.TAVILY_API_KEY:
            st.error("Bitte TAVILY_API_KEY in den Umgebungsvariablen setzen")
            return
